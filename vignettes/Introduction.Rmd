...
```{r,echo=F}
rsstap_demo$subjectbef_positions %>% 
...
```{r,echo=F}
rsstap_demo$direct_distance_data %>% head() %>%  knitr::kable()
```
...
tibble(Distance = seq(from = 0, to = 2.5,by=0.01),
       Effect = rsstap_demo$true_effect(Distance)) %>% 
...
```
  ...
  We'll finish our simulation of the data by including a 0-1 sex covariate to make things a little more realistic - the code containing all the simulation details can be found [here](https://github.com/apeterson91/rsstap), and the resulting dataframe will look like the following.
  
```{r,echo=F}
rsstap_demo$subject_data %>% head() %>% knitr::kable()
```
...
```{r}
restricted_distance <- rsstap_demo$direct_distance_data %>% filter(Distance<=2)

demo_knots <- quantile(restricted_distance$Distance,
                  probs = c(0.25,.75))
```
# Model Fitting
Finally we'll fit our model, using syntax similar to the `lm` function, where the `~ sap(HFS)` indicates
...
```{r}
fit <- rsstap_lm(formula = y ~ sex,
               stap_formula = ~ sap(HFS),
               subject_data = rsstap_demo$subject_data,
               subject_id = "subj_id",
               basis_functions = list(HFS= function(x){ splines::ns(x,knots = demo_knots,
                                                                    Boundary.knots = c(0,2)) }),
               dt_data = restricted_distance,
               BEF_col_name = "BEF",
               distance_col_name = "Distance",method='lm')

summary(fit)
```
As we look at the output, we can see this is similar to the `lm` function's summary output. This makes sense because the 
object returned by the `sstap_` functions inherit the class of whatever function was chosen using the `method` argument - in this case the `lm` function.

It also inherits a new `rsstap` class which allows for custom methods specific to these objects. For example, if we want to to plot the 
spatial exposure estimate we can do so using the `plot_effects` function. I'll overlay the true exposure effect we simulated for comparison.

```{r}
d <- seq(from = min(restricted_distance$Distance), to = 2, by =0.01)
plot_effects(fit) + ggplot2::theme_bw() +
  ggplot2::geom_line(ggplot2::aes(x=d,y=rsstap_demo$true_effect(d)),color='red') + 
  theme(strip.background = element_blank()) + xlab("Distance")
```
...
we can simply use the ``sstap_lm` function setting the `method` argument to either `"stan_lm"` or `"brm"`, to use the function from the rstanarm or brms packages respectively. Note that now we 
...
This vignette briefly illustrates the `rsstap` modeling framework and typical function uses. Future vignettes will cover the 
