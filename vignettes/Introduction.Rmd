...
object returned by the `bbnet_` functions inherit the class of whatever function was chosen using the `method` argument - in this case the `lm` function.
...
we can simply use the ``bbnet_lm` function setting the `method` argument to either `"stan_lm"` or `"brm"`, to use the function from the rstanarm or brms packages respectively. Note that now we 
