...
In the `rsstap` Introductory [vignette](https://apeterson91.github.io/rsstap/articles/Introduction.html), we introduced the STAP formulation of models implimented using splines 
and demonstrated how the `rsstap` package uses [`rstan`](https://mc-stan.org/users/interfaces/rstan) and [`mgcv`](https://cran.r-project.org/web/packages/mgcv/index.html) to fit these models in a context where observations are modeled independent of one another. 
In this vignette I'll introduce the longitudinal modeling framework in `rsstap` that incorporates the familiar [`lme4`](https://github.com/lme4/lme4) model formula syntax in addition to 
...
bdf
```
Looking at descriptives of this exposure over time, we can see that there is a meaningful change in the number of distances across the measurement visits. (Foreshadowing)  This will be an important fact to consider in our modeling approach going forward.
```{r bdf_descriptive2}
bef_summary(bdf)
```


We'll fit this model using the included sex and year information, modeling the spatial temporal exposure to HFS' through a tensor spline basis function expansion and two subject specific terms accounting for within subject outcome correlation. 
...
akin to the standard formulation as in the `lme4` package and 

$$
f(\text{HFS Exposure}_i) = \sum_{l=1}^{L}\sum_{(d,t) \in\mathcal{S}} \beta_l\phi_l(d,t),
$$

with $\phi_l(\cdot)$ the basis function evaluation.

In order to fit this model in `rsstap` we can use the code below, setting the optional sampler arguments for `stan` similar to how was demonstrated in the Introductory vignette.
...
Finally we'll look at a 3d plot and 2d plot cross sections of the HFS effect across space and time. This will allow us
to establish whether there is any effect at all, and what it looks like (as a median estimate).
